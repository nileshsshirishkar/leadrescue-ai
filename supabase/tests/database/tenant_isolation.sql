...
select extensions.results_eq($$select count(*)::bigint from public.organizations$$,$$values (1::bigint)$$,'Tenant A sees only its organization');
select extensions.results_eq($$select count(*)::bigint from public.contacts$$,$$values (1::bigint)$$,'Tenant A sees only its contact');
select extensions.results_eq($$select count(*)::bigint from public.leads$$,$$values (1::bigint)$$,'Tenant A sees only its lead');
select extensions.results_eq($$select count(*)::bigint from public.profiles$$,$$values (1::bigint)$$,'Tenant A sees only profiles in its organization');
...
select extensions.throws_ok(
  ...
  '42501','new row violates row-level security policy for table "contacts"','Tenant A cannot insert a contact into Tenant B'
);
...
select extensions.throws_ok(
  $$update public.organizations set name = 'Unauthorized' where id = '10000000-0000-0000-0000-000000000001'$$,
  '42501','permission denied for table organizations','Authenticated clients cannot update organization settings'
);
...
select extensions.throws_ok(
  ...
  'P0001','organization_id is immutable','Tenant-owned records cannot be moved between organizations'
);
...
select extensions.results_eq($$select count(*)::bigint from public.organizations$$,$$values (1::bigint)$$,'Tenant B sees only its organization');
select extensions.results_eq($$select count(*)::bigint from public.leads$$,$$values (1::bigint)$$,'Tenant B sees only its lead');
select extensions.results_eq(
  ...
  $$values (1::bigint)$$,
  'Owner can delete a lead in its own active organization'
);
