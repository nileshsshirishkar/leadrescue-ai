...
const FIELD_ALIASES: Record<keyof Lead, string[]> = {
  ...
  sourceStage: ["status", "leadstatus", "pipelinestatus", "stage", "sourcestage", "originalstage", "externalstage"],
  status: ["leadrescuestatus", "workflowstatus"],
...
export function normalizeCsvRows(rows: Record<string, unknown>[]): CsvNormalizationResult {
  ...
  rows.forEach((row, index) => {
    ...
    const candidate = {
      ...
      sourceStage: cleanText(normalized.sourceStage) || undefined,
      status: "New",
