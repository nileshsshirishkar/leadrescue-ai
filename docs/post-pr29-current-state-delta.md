...
## Current vendor/security facts reverified 02 September 2026

- Connected Vercel team `LeadRescue AI` remains on the `hobby` plan.
- Current Vercel documentation states rollback to a specific older Production deployment is available on Pro or Enterprise plans.
- Supabase currently documents leaked-password protection as available on Pro and above.
- Supabase currently documents automatic daily database backups for Pro, Team and Enterprise; Pro has access to the last seven days of daily backups.
- PITR is a separate paid add-on and should not be enabled by assumption. The approved Client #1 RPO target is up to 24 hours, so daily Pro backups remain the current baseline unless later restore testing or business requirements justify a lower RPO.
- Supabase restore-to-new-project is available on paid plans when physical backups are enabled and is suitable for the first controlled restore verification because it can prove recoverability without overwriting the active Production database.
